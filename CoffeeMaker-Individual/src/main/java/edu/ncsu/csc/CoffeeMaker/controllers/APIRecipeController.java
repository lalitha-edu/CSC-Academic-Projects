...
public class APIRecipeController extends APIController {
    ...
    // /**
    // * InventoryService object, to be autowired in by Spring
    // */
    // @Autowired
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        if ( null != service.findByName( recipe.getName() ) ) {
            return new ResponseEntity( errorResponse( "Recipe with the name already exists" ), HttpStatus.CONFLICT );
        }
        if ( service.findAll().size() < 3 ) {

            if ( recipe.getPrice() == null || recipe.getPrice() < 0 ) {
                return new ResponseEntity( errorResponse( "Price must be a positive value" ), HttpStatus.CONFLICT );
            }
            else if ( recipe.getIngredients().size() <= 0 ) {
                return new ResponseEntity( errorResponse( "Insufficient ingredients count" ),
                        HttpStatus.INSUFFICIENT_STORAGE );
            }
            else if ( recipe.getIngredients().size() > 0 ) {

                for ( int i = 0; i < recipe.getIngredients().size(); i++ ) {
                    if ( null == recipe.getIngredients().get( i ).getAmount()
                            || 0 > recipe.getIngredients().get( i ).getAmount() ) {
                        return new ResponseEntity( errorResponse( "Amount must be a positive value" ),
                                HttpStatus.CONFLICT );
                    }
            ...
            return new ResponseEntity( errorResponse( "Insufficient space in recipe book for recipe" ),
                    HttpStatus.INSUFFICIENT_STORAGE );
        }
    ...
    /**
     * REST API method to allow update a Recipe from the CoffeeMaker's
    ...
    public ResponseEntity editRecipe ( @RequestBody final Recipe recipe ) {
        ...
        if ( recipe.getPrice() == null || recipe.getPrice() < 0 ) {
            ...
            for ( int i = 0; i < recipe.getIngredients().size(); i++ ) {
                if ( 0 > recipe.getIngredients().get( i ).getAmount() ) {
                    return new ResponseEntity( errorResponse( "Amount must be a positive value" ),
                            HttpStatus.CONFLICT );
                }
